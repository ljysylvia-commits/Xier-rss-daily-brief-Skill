...
读入 clusters_index.json（所有 cluster 的轻量索引：标题、信源、content_type、language、member_count、earliest_published_at、rss_summary、content_excerpt、primary_tokens、fetch_status、evidence_state）。
...
- `primary_tokens=0 && rss_summary=null && fetch_status=content_extraction_failed` 时必须 `tier=others` 且 `final_score=0`
- `low_density_suspect` 字段**不与 tier 做一致性检查**（Scorer 只标注；降档决策交 VM 阶段）

Lint 失败：重试 1 次；仍失败则主 Agent 把有证据但有问题的 cluster 强制置为 `optional` 并记录 warning。若 cluster 满足 `primary_tokens=0 && rss_summary=null && fetch_status=content_extraction_failed`，主 Agent 必须运行 deterministic guardrail 强制置为 `others`。
