...
| 模式 | 使用场景 | 执行步骤 |
|---|---|---|
| `setup` | 首次配置用户，或 demo 文件仍需替换 | Step -2、Step -1，然后 Step 0-6 |
| `tuning` | 用户 review 后要求调整评分、信源、价值视角或视觉样式 | Step 7，然后重跑 Step 0-6 |
| `stable` | 配置已确认，日报应每日运行 | 只跑 Step 0-6 |
