...
| `stable` | 用户已确认配置，可每日自动运行 | 只跑 Step 0-6（包含 Step 4d） |

Step -2、Step -1、Step 7、Step 8 不是每日自动化步骤。stable 模式下，除非文件缺失、healthcheck 失败，或用户明确要求修改画像、信源、评分、反馈或自动化配置，否则只执行 Step 0-6（含 Step 4d）。
...
RSS / Web 中出现 X/Twitter URL 时也不进入日报。Fetcher / source boundary 会记录 `unsupported_source_type`，并跳过该条目，不写入 `raw_items.jsonl`；source 级 URL 会在信源状态中显示为“不支持的信源”。
...
Scorer 输出后必须运行确定性 guardrail，保证无正文证据内容固定落到 `others`：
...
### Step 4d · Final Others 中文摘要

Step 4c 后再生成最终 Other Signals 输入包，确保 `scored.json` 中原始 `tier=others` 的条目、VM 发现 `title_content_mismatch` 后自动降级到 others 的条目，以及 `report_config.report_length` 触发长度上限后下放到 Other Signals 的条目，都被同一套 `gist_zh` 合同覆盖。
...
- system_prompt = `references/agents/other-signal-summarizer.md §3`
- user_message = `"${RUN_DIR}/other_signal_inputs.json"`
- output_file = `"${RUN_DIR}/others_translated.json"`（由主 Agent 校验 JSON 后写入）
- `others_translated.json` 只输出 `cluster_id`、`title_zh`、`gist_zh`；来源、链接、备注由 Writer 从上游 metadata 合并
- `gist_zh` 必须是中文，≤ 80 中文字；不得直接截取英文原文
- `primary_tokens=0`、`rss_summary=null`、`fetch_status="content_extraction_failed"` 的条目固定输出 `gist_zh = "中文摘要缺失"`，不得生成事实摘要
- 有可用证据的条目不得使用 `gist_zh = "中文摘要缺失"`
- Writer 渲染「其他信息」时必须使用该文件；缺失、漏条、额外条、质量校验失败时中止，不让英文 raw gist 或 VM fallback 进入日报
